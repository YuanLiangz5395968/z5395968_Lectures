...
#test
#print(ROOTDIR)     ###Successful
print(DATDIR)      ###Successful
#print(TICPATH)     ###Successful
# ----------------------------------------------------------------------------
...
def get_tics(pth):
    ...
    with open(pth, 'r') as file:
        res = {}
        # read first line data of ticker
        content = file.readline()
        content = content.lower()
        while content:
            # ticker and exchange is separated by '='
            lst = content.strip().replace('\"','').split('=')
            exchange = lst[0]
            ticker = lst[1]
            res[ticker] = exchange
            # read next line of ticker file
            content = file.readline()
...
def read_dat(tic):
    ...
    pth = os.path.join(DATDIR, f'{tic}_prc.dat')    #Avoid absolute paths
    #test
    #print(pth)
    # <COMPLETE THIS PART>

    # list for storing all data meet the demand
    lst = []
    # all files under DATDIR path
    file_names = os.listdir(DATDIR)
    for file_name in file_names:
        if file_name.find(tic) != -1:
            # recover the full path of .dat file
            path = os.path.join(DATDIR, file_name)
            # open .dat file found above
            with open(path, 'r') as dat:
                ...
                while line:
                    ...
                    lst.append(line)
                    # read next line without whitespace
                    line = dat.readline()
                    line = line.strip()
    return lst
...
def line_to_dict(line):
    ...
    res = {}
    # variable for start and end index for substring in line data
    start = 0
    end = 0
    for col in COLUMNS:
        # width attribute defined in COLWIDTHS
        width = COLWIDTHS[col]
        end = start + width
        data = line[start:end]
        # eliminate white space in Date attribute
        data = data.strip()
        # update start and end index for next substring
        start = end
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    # <COMPLETE THIS PART>
    # list is empty
    if tickers_lst is None or len(tickers_lst) == 0:
        raise Exception
    # tickers_lst doesn't contain the ticker
    for ticker in tickers_lst:
        # ticker is not in tic_exchange_dic
...
def verify_cols(col_lst=None):
    ...
    # <COMPLETE THIS PART>
    # lst is empty
    if col_lst is None or len(col_lst) == 0:
        raise Exception
    for col in col_lst:
        # col_list is not in columns
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    """
    # <COMPLETE THIS PART>
    # make sure parameters are not empty before creating dict
    verify_tickers(tic_exchange_dic, tickers_lst)
    verify_cols(col_lst)

    # all data would be stored in res dict
    res = {}
    # first of all, read ticker from ticker list
    for ticker in tickers_lst:
        dat_list = read_dat(ticker)
        # all data belong to current ticker would be stored in this list
        data = []
        for dat in dat_list:
            # dict for one line data in current dat file
            dic = {}
            one_line = line_to_dict(dat)
            # not all data would be printed to screen
            for col in col_lst:
                dic[col] = one_line[col]
            data.append(dic)
        # insert item to dict
        res[ticker] = {'exchange': ticker, 'data': data}
    return res
...
def create_json(data_dict, pth):
    ...
    """
    # <COMPLETE THIS PART>
    # dump function generate a json file whereas dunps function generate a json string
    # dump() is the best solution for this function
